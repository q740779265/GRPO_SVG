...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        with init_context(), warnings.catch_warnings():
            ...
            #         logger.info(f'use lora config: {lora_config}')
            #         actor_module = get_peft_model(actor_module, LoraConfig(**lora_config))
            #     else:
            #         logger.info(f'use default lora config')
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            ...
            self.actor_module_fsdp, self.actor_optimizer, self.actor_lr_scheduler, self.actor_model_config = self._build_model_optimizer(
                ...
                lora_config=self.config.model.lora_config if self.config.model.get('use_lora', False) else None)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        if self.world_size > 1:
            if hasattr(self.actor.actor_module, '_handle') and self.actor.actor_module._handle is not None:
