...
# from verl.utils.reward_score.svg import compute_score
import ray
import hydra
import subprocess
import os
...
def main():
    ...
    cli_config = '/home/wxf/lzq/twq/verl-main/verl/trainer/config/grpo_trainer.yaml'
    config = OmegaConf.load(cli_config)
    # config = OmegaConf.merge(file_config, cli_config)
    if not ray.is_initialized():
        # this is for local ray cluster
        ray.init(runtime_env={'env_vars': {'TOKENIZERS_PARALLELISM': 'true', 'NCCL_DEBUG': 'WARN'}})
    # main_task(config)
    # os.environ['WORLD_SIZE'] = str(config.trainer.nnodes * config.trainer.n_gpus_per_node)
    # os.environ['RANK'] = '0'
...
def main_task(config, compute_score=None):
    ...
    resource_pool_spec = {
        ...
        # global_pool_id_1: [config.trainer.n_gpus_per_node // 2] * config.trainer.nnodes,
    }
    mapping = {
