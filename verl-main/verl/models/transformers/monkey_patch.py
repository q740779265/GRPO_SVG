...
def apply_monkey_patch_to_gemma2():
    from transformers.models.gemma2.modeling_gemma2 import Gemma2FlashAttention2
    from verl.models.transformers.gemma2 import gemma2_flash_attn_forward
    Gemma2FlashAttention2.forward = gemma2_flash_attn_forward


_PATCH_NAME_TO_FUNC = {
    'llama': apply_monkey_patch_to_llama,
    'qwen2': apply_monkey_patch_to_qwen2,
    # 'gemma2': apply_monkey_patch_to_gemma2
}
...
def apply_monkey_patch(config: PretrainedConfig, verbose=True):
    '''将transformers特定模型的前向传播函数修改为自定义的函数'''
