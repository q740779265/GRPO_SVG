...
class custom_manager:
    ...
    def score(self, submission: str, ground_truth: str) -> float:
        # print("正在计算score此时的prompt: ",ground_truth)
        constraints = SVGConstraints()
        try:
            svg_code = re.findall(r'<svg[^>]*>[\s\S]*?<\/svg>', submission)[-1]
        
            # print("生成的代码: \n",svg_code)
            if not constraints.validate_svg(svg_code):
                return 0.0
            # print("正在转化到PNG")
            image = svg_to_png(svg_code)
            # print("转化成功")
            # print("图像：", image)
            vqa_score = p_fidelity = self.vqa_evaluator.score(image, ground_truth)
...
class VQAEvaluator:
    ...
    def __init__(self):
        ...
        self.model_path = '/home/wxf/lzq/data/model/paligemma2-10b-mix-448/models--google--paligemma2-10b-mix-448/snapshots/b26d16fb4251090ba4a4aa5af9fca1f8248ed5b6'
        self.processor = AutoProcessor.from_pretrained(self.model_path, device_map='cuda:1')
        ...
        self.questions = {
            'fidelity': 'Does portray "{}" without any lettering? Answer yes or no.',
            'text': 'Text present: yes or no?',
        }
    ...
    def score(self, image: Image.Image, description: str) -> float:
        ...
        """
        # print('prompt:', self.questions['fidelity'].format(description))
        p_fidelity = self.get_yes_probability(image, self.questions['fidelity'].format(description))
        # print('prompt:', self.questions['text'])
    ...
    def get_yes_probability(self, image, prompt) -> float:
        inputs = self.processor(images=image, text=prompt, return_tensors='pt').to('cuda:1')
        # print("CUDA_VISIBLE_DEVICES: ",os.environ['CUDA_VISIBLE_DEVICES'])
        with torch.no_grad():
